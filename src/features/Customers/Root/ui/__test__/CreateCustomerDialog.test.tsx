...
describe('CreateCustomerDialog', () => {
  it('+ Add CustomerボタンをクリックするとonOpenが呼ばれること', async () => {
  ...
  it('errors.commonが表示されること', () => {
    renderDialog({ errors: { common: '顧客の登録に失敗しました' } })

    expect(screen.getByText('顧客の登録に失敗しました')).toBeInTheDocument()
  })
  ...
  it('isPendingがtrueの場合、Saveボタンがloading状態になること', () => {
    renderDialog({ isPending: true })

    // ローディング中はラベルが隠れるため、DOM順（+Add Customer, Cancel, Save）で判定する
